...
/**
 * check 67 — GRAPHICS pack (motion designer): animated shapes, CTAs, lower thirds,
...
async function checkGraphicsPack(): Promise<void> {
  ...
  const info = await detectFfmpeg();
  if (!info.available) {
    console.log(`  \x1b[32m✔\x1b[0m check 67 (graphics pack): 5 groups animate/settle/loop; windows drive export; plain shapes static — ffmpeg unavailable, encode skipped`);
    return;
  }
  const bin = resolveFfmpegBin();
  const encDir = resolve(OUT_DIR, "encode");
  ...
  console.log(
    `  \x1b[32m✔\x1b[0m check 67 (graphics pack): 5 groups (CTA, lower third, timer, progress, badge) pop in → settle → loop; timer ticks as ${tw.length} windows; plain shapes stay static; edits rebuild in place; media-less export matches the canvas (mean |ΔRGB| ${diffs.join("/")}); over footage ${seqs} PNG sequences + stills animate on export`,
  );
}
